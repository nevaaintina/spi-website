...
export default function AdminLayout({ children, currentPage }) {
  ...
  const menuPages = [
    ...
    { id: 'about', label: 'About Us', path: '/admin/about-manager', icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>
    )},
    { id: 'products', label: 'Products & Equipment', path: '/admin/products', icon: (
    ...
    { id: 'parts-manager', label: 'Spare Parts & Catalog', path: '/admin/spare-parts-manager', icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>
    )},
    { id: 'knowledge', label: 'Knowledge Center', path: '/admin/knowledge', icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
    )},
    { id: 'media', label: 'Media Gallery', path: '/admin/media', icon: (
      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
    )},
    { id: 'career', label: 'Career', path: '/admin/career', icon: (
    ...
    { id: 'contact', label: 'Contact Us', path: '/admin/contact', icon: (
          ...
          <nav className="p-4 space-y-1 text-xs font-semibold overflow-y-auto max-h-[calc(100vh-160px)]">
            ...
            {menuPages.map((menu) => (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition cursor-pointer ${currentPage === menu.id ? 'bg-[#ffc107] text-[#0f2b5c] font-black shadow-lg' : 'text-slate-300 hover:bg-white/5 hover:text-white'}`}
              >
